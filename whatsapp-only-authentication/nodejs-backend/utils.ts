  ...
  try {
    ...
    if (!res.ok) {
      ...
      const error = new Error(
        `WhatsApp send failed - Status: ${res.status}`
...
export function logReturn(tag: string, event: any, extra?: any) {
  const safeOut = {
    userName: event.userName,
    trigger: event.triggerSource || event.trigger,
    sessionLen: event.request?.session?.length || 0,
    response: { ...event.response },
    ...(extra ? { extra } : {}),
  };
  console.log('[RETURN]', tag, safeOut);
  return event; // do not mutate event.response here
}
