...
package "Error Sources" {
  [User Input Errors] as UserInput
  [Network Errors] as Network
  [API Errors] as API
  [System Errors] as System
}

package "Error Detection" {
  [Input Validation] as Validation
  [Network Monitoring] as NetworkCheck
  [API Response Check] as APIResponse
  [System Health Check] as SystemCheck
}

package "Error Processing" {
  [Error Categorization] as Categorization
  [Error Logging] as Logging
  [User Message Generation] as UserMessage
  [Recovery Actions] as Recovery
}

package "Error Display" {
  [Error UI Components] as ErrorUI
  [Help Text] as HelpText
  [Action Buttons] as ActionButtons
