...
export default function Home() {
  ...
  const handleSignupSuccess = async (phoneNumber: string) => {
    try {
      console.log('Signup successful, triggering OTP for:', phoneNumber);
      
      // Trigger custom auth challenge for OTP
      const result = await authService.triggerSignupOTP(phoneNumber);
      console.log('OTP trigger result:', result);
      
      toast.success(result.message);
      setUserData({ phoneNumber });
      setCurrentState('signup-otp');
      console.log('Navigating to signup-otp state');
    } catch (error: any) {
